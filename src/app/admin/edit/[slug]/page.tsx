...
export default function AdminEditProposalPage() {
  ...
  const [existingPages, setExistingPages] = useState<{ src: string; alt: string }[]>([]);
  ...
  useEffect(() => {
    fetch(`/api/admin/proposals/${slug}`)
      .then((res) => {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      let pages: { src: string; alt: string }[] | undefined;

      // 1. Upload new PDF if changed
      if (pdfFile) {
      ...
      // 2. Upload new page images if changed
      ...
      const res = await fetch(`/api/admin/proposals/${slug}`, {
        method: 'PUT',
    ...
    return (
      <>
        <div className={styles.header}>
          <h1 className={styles.headerTitle}>제안서 수정</h1>
        </div>
  ...
  return (
    <>
      <div className={styles.header}>
        <h1 className={styles.headerTitle}>제안서 수정: {slug}</h1>
        <div className={styles.headerNav}>
          <Link href="/admin" className={styles.navLink}>대시보드</Link>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* PDF Upload (optional for edit) */}
        <div className={`${styles.inputGroup} ${styles.formFull}`} style={{ marginTop: '1rem' }}>
          <label className={styles.label}>PDF 파일 (변경 시에만 업로드)</label>
          <div
        ...
        {/* New page previews (from new PDF) */}
        {pagePreviews.length > 0 && (
          <div style={{ marginTop: '1rem' }}>
            <label className={styles.label}>새로 변환된 페이지 ({pagePreviews.length}페이지)</label>
        ...
        {/* Existing page previews (if no new PDF uploaded) */}
        {existingPages.length > 0 && pagePreviews.length === 0 && (
          <div style={{ marginTop: '1rem' }}>
            <label className={styles.label}>현재 페이지 ({existingPages.length}페이지)</label>
            <div className={styles.previewGrid}>
        ...
        <div style={{ marginTop: '1.5rem' }}>
          ...
          >
            {isSubmitting ? uploadProgress || '저장 중...' : '수정 저장'}
          </button>
